...
$stmt = $pdo->prepare("SELECT * FROM celestial_bodies");
$stmt->execute();

// Fetch all the planet records
$celestial_bodies = $stmt->fetchAll();
?>
...
<body>
  ...
  <main>
    ...
    <section class="intro">
      ...
      <a href='list_celestial_bodies.php' class="start-button">Start Adventure</a>
    </section>
    <!-- Gallery -->
    ...
    <div class="article-container">
        <?php if (!empty($celestial_bodies)): ?>
            <?php foreach ($celestial_bodies as $celestial_body): ?>
                <article>
                    <img src="<?php echo htmlspecialchars($celestial_body['image_url']); ?>" alt="<?php echo htmlspecialchars($celestial_body['name']); ?>">
                    <h3><?php echo htmlspecialchars($celestial_body['name']); ?></h3>
                    <p><?php echo htmlspecialchars($celestial_body['description']); ?></p>
                </article>
            <?php endforeach; ?>
        <?php else: ?>
            <p>No result found.</p>
